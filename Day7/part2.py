...
class Node:
    def __init__(self, value, list):
        ...
        if len(list) > 1:
            ...
            self.concatenate = Node(int(str(value) + str(list[0])), list[1:])
        elif len(list) == 1:
            self.multiply = Node(value * list[0], [])
            self.add = Node(value + list[0], [])
            self.concatenate = Node(int(str(value) + str(list[0])), [])
        else:
            self.multiply = None
            self.add = None
    ...
    def is_leaf(self):
        return self.multiply == None and self.add == None and self.concatenate == None
    
def explore_tree(tree, to_find):
    if tree.is_leaf():
        return tree.value == to_find
    return explore_tree(tree.multiply, to_find) or explore_tree(tree.add, to_find) or explore_tree(tree.concatenate, to_find)
